...
def find_brightest_UKIRT(table, bh_coord):
    ...
    k_coord = SkyCoord(float(best['ra']), float(best['dec']), unit='deg', frame='icrs')
    ...
    for bh in black_holes:
        ...
        if (table is None) or (len(table) == 0):
            ...
            results.append({
                ...
                'bh_ra': bh_ra,
                'bh_dec': bh_dec,
                'star_ra': "Nothing found",
                'star_dec': "Nothing found",
                'sep_arcsec': "Nothing found",
                'k_mag': "Nothing found",
                'k_mag_err': "Nothing found"
            })
            continue
